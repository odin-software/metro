class World {
  ...
  #generateBuildings() {
    ...
    for (let i = 0; i < bases.length - 1; i++) {
      for (let j = i + 1; j < bases.length; j++) {
        const base1 = bases[i];
        const base2 = bases[j];
        if (base1.intersectsPoly(base2) || base1.distanceToPoly(base2) < this.spacing - eps) {
          bases.splice(j, 1);
          j--;
        }
      }
  ...
  #generateTrees() {
    ...
    while (tryCount < 80) {
      ...
      // chcking if trees are inside illegal polygons
      let keep = true;
      ...
      if (keep) {
        for (const tree of trees) {
          if (p.distanceTo(tree.center) < this.treeSize) {
            keep = false;
            break;
          }
        }
  ...
  draw(ctx, viewPoint, showStartMarkings = true, renderRadius = 2000) {
    ...
    const items = [...this.buildings, ...this.trees].filter(item => item.base.distanceToPoint(viewPoint) < renderRadius);
